...
public class ContainerGlowstoneCharger extends Container
{
    private final TileEntityGlowstoneCharger tileentity;
    private int energy, cookTime;

    public ContainerGlowstoneCharger(InventoryPlayer player, TileEntityGlowstoneCharger tileentity)
    {
        this.tileentity = tileentity;
        IItemHandler handler = tileentity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null);

        this.addSlotToContainer(new SlotItemHandler(handler, 0, 80, 33));
        this.addSlotToContainer(new SlotItemHandler(handler, 1, 152, 33));
